...
interface IBARBELLERC20 is IERC20Mintable, IERC20 {
...
contract BarbellTreasury is Ownable {
    ...
    IBARBELLERC20 public immutable Barbell;
    ...
    constructor (
        ...
        Barbell = IBARBELLERC20(_Barbell);

        isReserveToken[ _FLEXUSD ] = true;
        reserveTokens.push( _FLEXUSD );
    ...
    /**
        @notice allow approved address to deposit an asset for Barbell
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Barbell needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Barbell.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Barbell for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Barbell.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = Muscle.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Barbell
        @param _amount uint
     */
    function repayDebtWithBarbell( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Barbell.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Barbell), _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        Barbell.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Barbell.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Barbell valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Barbell decimals
            value_ = _amount.mul( 10 ** Barbell.decimals() ).div( 10 ** IERC20( _token ).decimals() );
