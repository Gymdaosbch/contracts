...
contract BarbellBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Barbell) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Barbell), "not a Barbell lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Barbell.decimals() ) ).div( getTotalValue( _pair ) );
    }
}
